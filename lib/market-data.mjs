...
export async function getMacroData() {
  ...
  const tasks = {
    ...
    spread10y2y: () => fetchFredSpreadOrYahoo("T10Y2Y", "^TNX", "2YY=F", "10Y-2Y"),
    spread10y3m: () => fetchFredSpreadOrYahoo("T10Y3M", "^TNX", "^IRX", "10Y-3M"),
...
async function fetchYahooYieldSpread(longSymbol, shortSymbol, label) {
  const [longYield, shortYield] = await Promise.all([
    fetchYahooQuote(longSymbol),
    fetchYahooQuote(shortSymbol),
  ]);
  const value = Number((longYield.value - shortYield.value).toFixed(2));
  const date = [longYield.date, shortYield.date].filter(Boolean).sort().at(-1) || null;
  return dataPoint(value, `Yahoo Finance fallback ${label}`, date);
}
...
async function fetchFredSpreadOrYahoo(seriesId, longSymbol, shortSymbol, label) {
  try {
    return await fetchFredLatestRecent(seriesId);
  } catch {
    return fetchYahooYieldSpread(longSymbol, shortSymbol, label);
  }
}
