...
export async function getMarketBasisData() {
  if (process.env.MOCK_MACRO === "1") {
    return {
      ...
      updatedAt: new Date().toISOString(),
      values: {
        korea: marketBasis("2021-06-25", 3316.08, "2022-09-30", 2134.77, "Mock"),
        us: marketBasis("2021-11-22", 16212.23, "2022-10-13", 10088.83, "Mock"),
      },
      errors: [],
    };
  }
  ...
  const entries = await Promise.all(
    markets.map(async ([key, symbol]) => {
      try {
        const basis = detectMajorVBottom(await fetchYahooHistory(symbol));
        if (!basis) throw new Error(`No confirmed major V-bottom for ${symbol}`);
        return [key, { ...basis, source: `Yahoo Finance ${symbol}` }, null];
      } catch (error) {
        return [key, null, String(error.message || error)];
      }
    }),
