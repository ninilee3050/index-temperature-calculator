  ...
  if (process.env.MOCK_MACRO === "1") {
    return {
      ...
      values: {
        ...
        us: {
          ...marketBasis("2021-11-22", 16212.23, "2022-10-13", 10088.83, "Mock"),
          risePeakDate: "2026-04-27",
          risePeakValue: 20177.66,
          fallPeakDate: "2022-10-13",
          fallPeakValue: 10088.83,
        },
      },
      errors: [],
    };
...
async function fetchYahooHistory(symbol) {
  ...
  return (result?.timestamp || [])
    ...
    .filter(
      (row) =>
        row.date && Number.isFinite(row.high) && Number.isFinite(row.low) && Number.isFinite(row.close),
    );
}
...
export function findBasisExtremes(dailyRows, basis) {
  const afterHigh = dailyRows.filter((row) => row.date >= basis.highDate);
  const afterLow = dailyRows.filter((row) => row.date >= basis.lowDate);
...
function aggregateMonthlyRows(dailyRows, asOfDate) {
  ...
  [...dailyRows]
    ...
    .forEach((row) => {
      if (!row.date || !Number.isFinite(row.high) || !Number.isFinite(row.low) || !Number.isFinite(row.close)) {
        return;
      }
