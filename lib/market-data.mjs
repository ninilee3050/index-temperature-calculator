...
async function fetchFredLatestRecent(seriesId) {
  try {
    ...
    const rows = await fetchFredSeries(seriesId, `&cosd=${start}`);
    for (let index = rows.length - 1; index >= 0; index -= 1) {
      const { date, value } = rows[index];
      if (date && Number.isFinite(value)) {
        return dataPoint(value, `FRED ${seriesId}`, date);
    ...
    return fetchFredLatestFromPage(seriesId);
  }
  return fetchFredLatestFromPage(seriesId);
}
...
async function fetchFredSeries(seriesId, query = "") {
  const csv = await fetchText(`https://fred.stlouisfed.org/graph/fredgraph.csv?id=${seriesId}${query}`);
  const rows = parseCsv(csv);
  return rows
    .slice(1)
